<script lang="ts">
  ...
  console.log("🚀  ~ file: PersonalInformation.svelte ~ line 18 ~ teacher", teacher);
  ...
  const save = async () => {
    ...
    promises.push(await api.teachers.update(teacher));
  ...
  const remove = async () => {
    await Promise.all([
      await api.phone.delete(person_id),
      await api.email.delete(person_id),
      await api.address.delete(person_id),
